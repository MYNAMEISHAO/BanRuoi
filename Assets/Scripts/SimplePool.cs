...
public static class SimplePool {
	...
	class Pool {
		...
		public TKey Spawn<TKey>(Vector3 pos, Quaternion rot)
		{
			GameObject obj;
			if (inactive.Count == 0)
			{
			...
			obj.transform.rotation = rot;
			obj.SetActive(true);
			return obj.GetComponent<TKey>();
		}
		public T Spawn<T>(Vector3 pos, Quaternion rot, Transform parent, bool isLocal = false)
		{
			GameObject obj;
			if (inactive.Count == 0)
			{
			...
			else
			{
				...
				if (obj == null)
				{
					return Spawn<T>(pos, rot, parent, isLocal);
				}
			}
		...
		public GameObject Spawn(Vector3 pos, Quaternion rot, Transform parent, bool isLocal = false)
		{
			GameObject obj;
			if (inactive.Count == 0)
			{
			...
			else
			{
				...
				if (obj == null)
				{
					return Spawn(pos, rot, parent, isLocal);
				}
			}
	...
	static public GameObject Spawn(GameObject prefab, Vector3 pos) => Spawn(prefab, pos, Quaternion.identity);
	...
	static public TKey Spawn<TKey>(GameObject prefab, Vector3 pos, Quaternion rot)
	{
		Init(prefab);
		return pools[prefab].Spawn<TKey>(pos, rot);
	}
	static public TKey Spawn<TKey>(GameObject prefab, Vector3 pos) => Spawn<TKey>(prefab, pos, Quaternion.identity);
	/// <summary>
	/// 
	/// </summary>
	/// <typeparam name="T">Return type of result object</typeparam>
	/// <param name="prefab"></param>
	/// <param name="pos"></param>
	/// <param name="rot"></param>
	/// <param name="parent">Transform to set parent object spawned.</param>
	/// <returns></returns>
	static public T Spawn<T>(GameObject prefab, Vector3 pos, Quaternion rot, Transform parent, bool isLocal = false)
	{
		Init(prefab);
		return pools[prefab].Spawn<T>(pos, rot, parent, isLocal);
	}
	static public GameObject Spawn(GameObject prefab, Vector3 pos, Quaternion rot, Transform parent, bool isLocal = false)
	{
		Init(prefab);
		return pools[prefab].Spawn(pos, rot, parent, isLocal);
	}
